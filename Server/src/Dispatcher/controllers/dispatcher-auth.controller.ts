...
import dispatcherAuthService = require('../services/dispatcher-auth.service');
//import { Request, Response, NextFunction } from 'express';
import * as bcrypt from 'bcrypt';
import { validate, validateOrReject } from 'class-validator';
import { SignInDto, SignUpDto } from '../../Customer/dto/customer-auth.dto';

export async function DispatcherSignUp(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  validate(signUpDto).then((errors) => {
    ...
    if (errors.length > 0) {
      ...
      dispatcherAuthService.sign_up(signUpDto);
      res.status(200).json({
...
export async function DispatcherSignin(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  validate(signInDto).then((errors) => {
    ...
    if (errors.length > 0) {
      ...
      dispatcherAuthService.sign_in(signInDto, res);
      //   res.status(200).json({
      //     message: 'Account Created Successfully!',
      //   });
    }
